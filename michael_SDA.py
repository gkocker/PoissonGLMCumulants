...
class SDA (object):
    def __init__(self,W=None,b=None,phi=None,tau=1.0):
        ...
        if W is not None:
            ...
            self.N = 100
            self.W = np.random.random([self.N,self.N])/self.N
    ...
    def compute_propagator_fourier(self):  #,omega=None):
        ...
        if not hasattr(self,'omega'):
            ...
            self.omega = np.linspace(0,10.0,500)
    ...
    def compute_summed_prop(self):
        ...
        self.prop_sum = np.sum(self.prop_fourier,axis=0)/self.N
    ...
    def compute_two_point_cor_population(self):
        ...
        for o in range(len(self.omega)):
            self.two_point_cor_population[o] = np.dot(self.prop_sum[:,o]*self.phi(self.steady_state), self.prop_sum[:,o].conj().T)
    ...
    def compute_three_point_population_A(self):
        ...
        # we assume that omega is [0,Omega) (non-negative and starts at 0)
        M=len(self.omega)
        m=np.min(1,M/2)  #we're going to assume that if len(M)==1 then self.omega==np.array([0.0])
        ...
        for o1 in range(m):
            for o2 in range(m):
                for r in range(self.N):
                    o3 = o1 + o2
                    self.three_point_cor_sum_A[o1,o2] += self.prop_sum[r,o1]*self.prop_sum[r,o2]*self.prop_sum[o3].conj()*phi[r]
    ...
    def compute_three_point_population_B(self):
        ...
        F_bar = np.tensordot(self.W,self.prop_fourier,axes=[[1],[0]])
        ...
        for o1 in range(m):
            ...
            for o2 in range(m):
                ...
                #o4 = o1 + o2
                for r in range(self.N):
                    factor1 = self.prop_sum[r,o1]*self.prop_sum[r,o2]
                    factor2 = self.prop_sum[r,o2]*self.prop_sum[r,o3]
                    factor3 = self.prop_sum[r,o1]*self.prop_sum[r,o3]
                    for s in range(self.N):
                        self.three_point_cor_sum_B[o1,o2] += factor1*self.prop_sum[s,o3]*F_bar[r,s,o3].conj()*phi[s]*phi_prime[r]
                        self.three_point_cor_sum_B[o1,o2] += factor2*self.prop_sum[s,o1]*F_bar[r,s,o1].conj()*phi[s]*phi_prime[r]
                        self.three_point_cor_sum_B[o1,o2] += factor3*self.prop_sum[s,o2]*F_bar[r,s,o2].conj()*phi[s]*phi_prime[r]
...
def main():
    ...
    sda_net = SDA()
    #l,v = np.linalg.eig(sda_net.W)

    s_0 = np.random.random(sda_net.N)
    sda_net.compute_mean_field_steady_state(s_0=s_0)
...
if __name__=='__main__':
    main()
    plt.figure(); plt.plot(sda_net.omega, sda_met.prop_fourier[0,1,:])