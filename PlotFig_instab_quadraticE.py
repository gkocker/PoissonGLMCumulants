...
if plot_raster:
    ...
    syn_scale = np.array((0., 4., 16.))  # for quadratic E, linear I
...
#     syn_scale = np.linspace(0, 100., Ncalc)  # for square root
    syn_scale = np.linspace(0., 12., Ncalc)  # for quadratic E, linear I

''' set save directory '''
if sys.platform == 'darwin': save_dir = '/Users/gocker/Documents/projects/structure_driven_activity/1loop_Ne=200_quadraticforEonly2/'
elif sys.platform == 'linux2': save_dir = '/local1/Documents/projects/structure_driven_activity/1loop_Ne=200_quadraticforEonly2/'
   ...
   #     print 'mft unstable, % complete = ' + str(float(nn) / float(Ncalc) * 100)
    for nt in range(Ntrials):
        ...
        if Ni > 0:
            ...
            two_point_Ipop_sim[nn, nt, :] = auto_covariance_pop(spktimes, range(Ne, N), spktimes.shape[0], dt, lags, tau,
                                                            tstop, trans)
...
if not plot_raster:
    ...
    if not 'linear' in save_dir:
        ax.plot(syn_scale[:Nstable], spec_rad_1loop[:Nstable],
