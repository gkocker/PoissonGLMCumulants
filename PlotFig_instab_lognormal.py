...
''' set save directory '''
if sys.platform == 'darwin': save_dir = '/Users/gocker/Documents/projects/structure_driven_activity/1loop_Ne=200_quadratic_transfer_lognormalWEE_nonER/'
elif sys.platform == 'linux2': save_dir = '/local1/Documents/projects/structure_driven_activity/1loop_Ne=200_quadratic_transfer_lognormalWEE_nonER/'
...
if os.path.exists(W0_path):
    ...
    W0EE = degdist(0, Ne, .8, -1.5, pEE, .8, Ne)
    W0[:Ne, :Ne] = W0EE
...
for nn in calc_range:
   ...
   #     print 'mft unstable, % complete = ' + str(float(nn) / float(Ncalc) * 100)
    for nt in range(Ntrials):
        ...
        r_readout_sim[nn, nt] = np.sum(spk_readout) / np.amax(spktimes[:, 0])
...
if not plot_raster:
    ...
    weight_list = W[:Ne, :Ne]
    syn_list = W0[:Ne, :Ne]
    ax.hist(weight_list[syn_list==1], bins=40, facecolor='k')
    ax.set_yscale('log')
    ax.set_xlabel('Exc-exc Synaptic Weight (mV)')
    ax.set_ylabel('Count')
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label]):
    ...
    savefile = os.path.join(save_dir, 'EEweight_histogram.pdf')
    fig.savefig(savefile)
    plt.show(fig)
    plt.close(fig)

    W0ER = np.load('/local1/Documents/projects/structure_driven_activity/1loop_Ne=200_quadratic_transfer/W0.npy')
    fig, ax = plt.subplots(1, figsize=(2,2))

    ax.plot(np.sum(W0[:Ne, :Ne], axis=1), np.sum(W0[:Ne, :Ne], axis=0), 'ko')
    ax.plot(np.sum(W0ER[:Ne, :Ne], axis=1), np.sum(W0ER[:Ne, :Ne], axis=0), 'go', label='Erdos-Renyi')
    ...
    savefile = os.path.join(save_dir, 'deg_dist.pdf')
    fig.savefig(savefile)
    plt.show(fig)
    plt.close(fig)
