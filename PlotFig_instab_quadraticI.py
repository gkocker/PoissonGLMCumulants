...
if plot_raster:
    # syn_scale = np.array((0., 8., 16.)) # for quadratic, was 75
    # syn_scale = np.array((0., 1., 12.)) # for linear
    # syn_scale = np.array((0., 1., 75.))  # for square root
    syn_scale = np.array((0., 20., 30.))  # for quadratic E, linear I
...
''' set save directory '''
if sys.platform == 'darwin': save_dir = '/Users/gocker/Documents/projects/structure_driven_activity/1loop_Ne=200_quadraticforIonly/'
elif sys.platform == 'linux2': save_dir = '/local1/Documents/projects/structure_driven_activity/1loop_Ne=200_quadraticforIonly/'
...
if not plot_raster:
    ...
    ax.set_ylim((0, rE_av_theory[Nstable-1]*2))
    ...
    if not 'linear' in save_dir:
        ax.plot(syn_scale[:Nstable], spec_rad_E_1loop[:Nstable], 'r', label='One loop', linewidth=2)
    ...
    if not 'linear' in save_dir:
        ax.plot(syn_scale[:Nstable], spec_rad_I_1loop[:Nstable], 'r', label='One loop', linewidth=2)
    ...
    savefile = os.path.join(save_dir, 'stability_vs_weight_Ionly.pdf')
    fig.savefig(savefile)
    plt.show(fig)
    plt.close(fig)
    ...
    ax.plot(lags, two_point_pop_sim[16, 0, :], 'g', label=r'$W_{EE} = 0.2$')
    ...
    ax.plot(lags, two_point_pop_sim[16, 0, :] / two_point_pop_sim[16, 0, ind_norm], 'g', label=r'$W_{EE} = 0.2$')
